        ...
        .CheckToken(token).then(async e => {
            ...
            try {
                ...
                findResults.forEach(fin => {
                    fin.borrow_start = moment(fin.borrow_start).format('LLL')
                    fin.borrow_end = moment(fin.borrow_end).format('LLL')
                })
                return res.send({
...
router.post("/borrow-details", async (req, res) => {
    ...
    `;
                ...
                return res.send({
                    ...
                    data: {
                        list: findResults, share: {
                            borrow_start: moment(borrow_start).format('LLL'), borrow_end: moment(borrow_end).format('LLL'), memo
                        },
                    }
                });
            } catch (e) {
